...
/**
 * Avaliação 360° - Etapa 1: Autoavaliação
 * 
 * Fluxo:
 * 1. Autoavaliação (colaborador) ← ESTA PÁGINA
...
export default function Avaliacao360Autoavaliacao() {
  ...
  const submitMutation = trpc.evaluation360.submitSelfAssessment.useMutation({
    onSuccess: () => {
      toast.success("Autoavaliação enviada com sucesso! O gestor foi notificado por email.");
      navigate("/avaliacoes");
    },
    onError: (error: any) => {
      toast.error(`Erro ao enviar autoavaliação: ${error.message}`);
    },
  });
  ...
  if (evaluation.workflowStatus !== "pending_self") {
    return (
      <DashboardLayout>
        <Card>
          <CardContent className="py-12 text-center">
            <h3 className="text-lg font-semibold mb-2">Autoavaliação já foi concluída</h3>
        ...
        <Card className="bg-blue-50 border-blue-200">
          <CardContent className="pt-6">
            ...
            <ul className="text-sm text-blue-800 space-y-1 list-disc list-inside">
              <li>Avalie seu próprio desempenho de forma honesta e objetiva</li>
              <li>Use a escala de 1 (Insatisfatório) a 5 (Excepcional)</li>
              <li>Responda todas as perguntas antes de enviar</li>
              <li>Após enviar, seu gestor será notificado por email para fazer a avaliação dele</li>
            </ul>
          </CardContent>
        </Card>
