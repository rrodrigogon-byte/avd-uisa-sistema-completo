...
export default function TestMBTI() {
  ...
  const { data: questions, isLoading } = trpc.psychometric.getQuestions.useQuery({ testType: "mbti" });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Teste MBTI enviado com sucesso!");
  ...
  const dimensionNames: Record<string, string> = {
    "E/I": "Extroversão vs Introversão",
    "S/N": "Sensação vs Intuição",
    "T/F": "Pensamento vs Sentimento",
    "J/P": "Julgamento vs Percepção",
  };
  ...
  return (
    <div className="container max-w-4xl py-8">
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>Teste MBTI - Myers-Briggs Type Indicator</CardTitle>
          <CardDescription>
            Descubra seu tipo de personalidade entre os 16 tipos do MBTI. Responda com sinceridade
            sobre como você geralmente se comporta.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className="space-y-6">
        {Object.entries(groupedQuestions).map(([dimension, qs]) => (
          <Card key={dimension}>
            <CardHeader>
              <CardTitle className="text-lg">{dimensionNames[dimension] || dimension}</CardTitle>
            </CardHeader>
      ...
      <div className="mt-8 flex justify-end">
        ...
        >
          {submitMutation.isPending ? "Enviando..." : "Enviar Teste MBTI"}
        </Button>
      </div>
    </div>
  );
