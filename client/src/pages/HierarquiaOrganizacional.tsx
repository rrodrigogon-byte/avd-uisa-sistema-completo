...
export default function HierarquiaOrganizacional() {
  ...
  // Queries
  const { data: hierarchy, isLoading: loadingHierarchy, refetch } = trpc.employees.getHierarchy.useQuery(undefined);
  const { data: departments } = trpc.employees.getDepartments.useQuery(undefined);
  const { data: managers } = trpc.employees.getManagers.useQuery(undefined);
