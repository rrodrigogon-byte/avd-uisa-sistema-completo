...
import { ChevronDown, ChevronRight, Users, Loader2, Building2, Search, Maximize2, Minimize2, Download, FileText, GripVertical } from "lucide-react";
...
export function OrganogramaDraggable() {
  const { data, isLoading, refetch } = trpc.orgChart.getOrgChart.useQuery(undefined);
  ...
  const exportToPNG = async () => {
    ...
    try {
      ...
      if (!element) {
        toast.error('Erro: Elemento do organograma não encontrado');
        return;
      }
      ...
      toast.error('Erro ao exportar organograma como PNG');
    } finally {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      toast.error('Erro ao exportar organograma como PDF');
    } finally {
      setIsExporting(false);
    }
  };
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold">Organograma Hierárquico (Drag & Drop)</h2>
              <p className="text-muted-foreground">
                Arraste funcionários para reorganizar a hierarquia
              </p>
            </div>
