...
--> statement-breakpoint
CREATE TABLE `customFieldHistory` (
	`id` int AUTO_INCREMENT NOT NULL,
	...
	`changedAt` timestamp NOT NULL DEFAULT (now()),
	CONSTRAINT `customFieldHistory_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `customFieldTemplates` (
	...
	`description` text,
	`fieldType` enum('text','textarea','number','date','select','multiselect','checkbox','radio','file','url','email') NOT NULL,
	`options` json,
...
CREATE TABLE `defaultApprovers` (
	...
	`workflowType` enum('job_description','pdi','bonus','evaluation','all') NOT NULL DEFAULT 'all',
	`isActive` boolean NOT NULL DEFAULT true,
	...
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `defaultApprovers_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE `pirEquipment` (
	`id` int AUTO_INCREMENT NOT NULL,
	`name` varchar(255) NOT NULL,
	`manufacturer` varchar(255),
	`model` varchar(255),
	`type` enum('half_mask','full_mask','n95','powered','supplied_air','scba') NOT NULL,
	`protectionFactor` int,
	`approvalNumber` varchar(100),
	`sizes` text,
	`isActive` boolean NOT NULL DEFAULT true,
	`stockQuantity` int DEFAULT 0,
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `pirEquipment_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `pirEquipmentUsage` (
	`id` int AUTO_INCREMENT NOT NULL,
	`employeeId` int NOT NULL,
	`equipmentId` int NOT NULL,
	`assignedDate` date NOT NULL,
	`returnedDate` date,
	`status` enum('in_use','returned','damaged','lost') NOT NULL DEFAULT 'in_use',
	`equipmentSize` varchar(50),
	`serialNumber` varchar(100),
	`assignedCondition` enum('new','good','fair') DEFAULT 'good',
	`returnedCondition` enum('good','fair','poor','damaged'),
	`notes` text,
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `pirEquipmentUsage_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `pirFitTests` (
	`id` int AUTO_INCREMENT NOT NULL,
	...
	`testType` enum('qualitative','quantitative') NOT NULL,
	`protocol` enum('osha_qualitative','osha_quantitative','iso_16975','nbr_15052') NOT NULL,
	`equipmentSize` varchar(50),
	`equipmentCondition` enum('new','good','fair','poor') DEFAULT 'good',
	`result` enum('pass','fail','inconclusive') NOT NULL,
	`overallFitFactor` decimal(10,2),
	`exerciseResults` text,
	`videoUrl` varchar(500),
	`videoS3Key` varchar(500),
	`videoDuration` int,
	`videoThumbnail` varchar(500),
	`testNotes` text,
	`testerName` varchar(255),
	`testerId` int,
	...
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `pirFitTests_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE `pirMedicalExams` (
	`id` int AUTO_INCREMENT NOT NULL,
	`employeeId` int NOT NULL,
	`programId` int,
	`examDate` date NOT NULL,
	`examType` enum('admissional','periodico','retorno_trabalho','mudanca_funcao','demissional') NOT NULL,
	`doctorName` varchar(255),
	`doctorCRM` varchar(50),
	`clinicName` varchar(255),
	`result` enum('apto','inapto','apto_com_restricoes') NOT NULL,
	`restrictions` text,
	`spirometry` text,
	`chestXray` text,
	`otherExams` text,
	...
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `pirMedicalExams_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `pirParticipants` (
	`id` int AUTO_INCREMENT NOT NULL,
	`programId` int NOT NULL,
	`employeeId` int NOT NULL,
	`status` enum('active','inactive','suspended','completed') NOT NULL DEFAULT 'active',
	`enrollmentDate` date NOT NULL,
	`lastTestDate` date,
	`nextTestDue` date,
	`medicalExamDate` date,
	`medicalExamStatus` enum('pending','approved','rejected','expired') NOT NULL DEFAULT 'pending',
	`medicalExamExpiry` date,
	`medicalNotes` text,
	`trainingDate` date,
	`trainingStatus` enum('pending','completed','expired') NOT NULL DEFAULT 'pending',
	`trainingExpiry` date,
	`trainingCertificate` varchar(500),
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `pirParticipants_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `pirPrograms` (
	`id` int AUTO_INCREMENT NOT NULL,
	`name` varchar(255) NOT NULL,
	`description` text,
	`startDate` date NOT NULL,
	...
	`coordinatorId` int,
	`coordinatorName` varchar(255),
	`requiresMedicalExam` boolean NOT NULL DEFAULT true,
	`requiresTraining` boolean NOT NULL DEFAULT true,
	`testFrequencyMonths` int NOT NULL DEFAULT 12,
	`createdBy` int,
	`createdByName` varchar(255),
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `pirPrograms_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE `pirTrainingParticipants` (
	`id` int AUTO_INCREMENT NOT NULL,
	`trainingId` int NOT NULL,
	`employeeId` int NOT NULL,
	`attendanceStatus` enum('registered','present','absent','excused') NOT NULL DEFAULT 'registered',
	`completionStatus` enum('pending','completed','failed') NOT NULL DEFAULT 'pending',
	`assessmentScore` int,
	...
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `pirTrainingParticipants_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `pirTrainings` (
	`id` int AUTO_INCREMENT NOT NULL,
	`title` varchar(255) NOT NULL,
	`description` text,
	...
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `pirTrainings_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE `positionFieldTemplates` (
	`id` int AUTO_INCREMENT NOT NULL,
	`positionId` int NOT NULL,
	`templateId` int NOT NULL,
	`required` boolean NOT NULL DEFAULT false,
	...
	`createdAt` timestamp NOT NULL DEFAULT (now()),
	`updatedAt` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `positionFieldTemplates_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
ALTER TABLE `users` ADD `faceRegisteredAt` datetime;--> statement-breakpoint
