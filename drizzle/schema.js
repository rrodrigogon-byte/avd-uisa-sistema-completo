...
import { boolean, date, datetime, int, json, mysqlEnum, mysqlTable, text, timestamp, varchar, } from "drizzle-orm/mysql-core";
...
export const videoMetadata = mysqlTable("videoMetadata", {
    ...
    multipleFacesTimestamps: json("multipleFacesTimestamps").$type(), // [10.5, 45.2, ...]
    noFaceTimestamps: json("noFaceTimestamps").$type(), // [5.1, 30.8, ...]
    personChangeTimestamps: json("personChangeTimestamps").$type(), // [20.3, ...]
    // Descritores faciais para comparação
    faceDescriptors: json("faceDescriptors").$type(), // Array de descritores ao longo do vídeo
    // Validação geral
...
export const pirQuestions = mysqlTable("pirQuestions", {
    ...
    options: json("options").$type(), // ["Opção 1", "Opção 2", ...]
    // Escala (para tipo escala)
    scaleMin: int("scaleMin"), // Ex: 1
    scaleMax: int("scaleMax"), // Ex: 5
    scaleLabels: json("scaleLabels").$type(), // ["Ruim", "Regular", "Bom", "Muito Bom", "Excelente"]
    // Configurações
    required: boolean("required").default(true).notNull(),
...
export const pdiResponsibilities = mysqlTable("pdiResponsibilities", {
    ...
    employeeResponsibilities: json("employeeResponsibilities").$type().default([]),
    ...
    leadershipResponsibilities: json("leadershipResponsibilities").$type().default([]),
    // Responsabilidades do DHO
    dhoTitle: varchar("dhoTitle", { length: 255 }).default("Responsabilidades do DHO (O Guardião)").notNull(),
    dhoResponsibilities: json("dhoResponsibilities").$type().default([]),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
/**
