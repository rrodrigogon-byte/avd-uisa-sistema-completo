...
// ============================================================================
// DESCRIÇÕES DE CARGO E APROVAÇÕES
// ============================================================================

// Imports para job descriptions e approvals já estão no início do arquivo
...
export async function listJobDescriptions(filters?: {
  ...
  if (filters?.status) {
    conditions.push(eq(jobDescriptions.status, filters.status as any));
  }

  if (filters?.level) {
    conditions.push(eq(jobDescriptions.level, filters.level as any));
  }

  if (filters?.department && filters.department.trim()) {
    const searchPattern = `%${filters.department.trim()}%`;
    conditions.push(sql`${jobDescriptions.department} LIKE ${searchPattern}`);
  }
  ...
  if (filters?.search && filters.search.trim()) {
    ...
    conditions.push(
      or(
        sql`${jobDescriptions.title} LIKE ${searchPattern}`,
        sql`${jobDescriptions.code} LIKE ${searchPattern}`
      )
    );
  }

  const query = db.select().from(jobDescriptions);
  const finalQuery = conditions.length > 0 ? query.where(and(...conditions)) : query;

  return await finalQuery.orderBy(desc(jobDescriptions.createdAt));
}
 ...
 */
export async function createJobDescription(data: InsertJobDescription) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
 ...
 */
export async function updateJobDescription(id: number, data: Partial<InsertJobDescription>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
/**
 * Atualizar status de descrição de cargo
 */
export async function updateJobDescriptionStatus(
  id: number,
  status: "rascunho" | "pendente_aprovacao" | "aprovado" | "rejeitado" | "arquivado",
  approvedBy?: number
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
 ...
 */
export async function createJobApproval(data: InsertJobApproval) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const result = await db.insert(jobApprovals).values(data);
  return result[0].insertId;
}
 ...
 */
export async function listPendingJobApprovals(approverId?: number) {
  const db = await getDb();
  if (!db) return [];

  const conditions = [eq(jobApprovals.status, "pendente")];

  if (approverId) {
    conditions.push(eq(jobApprovals.approverId, approverId));
  }
  ...
  return await db
    ...
    .from(jobApprovals)
    .where(and(...conditions))
    .orderBy(desc(jobApprovals.requestedAt));
}
...
export async function updateJobApprovalStatus(
  ...
  await db
    ...
    .where(eq(jobApprovals.id, id));
}

/**
 * Aprovar múltiplas descrições de cargo em lote
 */
export async function bulkApproveJobDescriptions(
  ...
  for (const jobId of jobDescriptionIds) {
    ...
    await updateJobDescriptionStatus(jobId, "aprovado", approverId);

    // Criar registro de aprovação
    const approvalId = await createJobApproval({
...
// ============================================================================
// TESTES DE INTEGRIDADE (EXPANSÃO DO PIR)
// ============================================================================

// Imports para integrity tests já estão no início do arquivo

/**
 * Listar testes de integridade disponíveis
 */
export async function listIntegrityTests(activeOnly = true) {
  const db = await getDb();
  if (!db) return [];

  const query = db.select().from(integrityTests);

  if (activeOnly) {
    return await query
      .where(eq(integrityTests.isActive, true))
      .orderBy(integrityTests.name);
  }

  return await query.orderBy(integrityTests.name);
}

/**
 * Buscar teste de integridade por ID
 */
export async function getIntegrityTestById(id: number) {
  const db = await getDb();
  if (!db) return null;
...
/**
 * Criar teste de integridade
 */
export async function createIntegrityTest(data: InsertIntegrityTest) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const result = await db.insert(integrityTests).values(data);
  return result[0].insertId;
}
 ...
 */
export async function updateIntegrityTest(id: number, data: Partial<InsertIntegrityTest>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  await db
    .update(integrityTests)
    .set({
      ...data,
      updatedAt: new Date(),
    })
...
/**
 * Criar resultado de teste de integridade
 */
export async function createIntegrityTestResult(data: InsertIntegrityTestResult) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const result = await db.insert(integrityTestResults).values(data);
  return result[0].insertId;
}
 ...
 */
export async function getEmployeeIntegrityTestResults(employeeId: number) {
  const db = await getDb();
  if (!db) return [];
  ...
  return await db
    ...
    .from(integrityTestResults)
    .innerJoin(integrityTests, eq(integrityTestResults.testId, integrityTests.id))
    .where(eq(integrityTestResults.employeeId, employeeId))
    .orderBy(desc(integrityTestResults.completedAt));
}

/**
 * Buscar resultado específico de teste
 */
export async function getIntegrityTestResultById(id: number) {
  const db = await getDb();
  if (!db) return null;
  ...
  const result = await db
    ...
    .from(integrityTestResults)
    .innerJoin(integrityTests, eq(integrityTestResults.testId, integrityTests.id))
    .where(eq(integrityTestResults.id, id))
    .limit(1);
 ...
 */
export async function getIntegrityAnalysis(employeeId: number, testId?: number) {
  const db = await getDb();
  if (!db) return [];

  const conditions = [eq(integrityTestResults.employeeId, employeeId)];

  if (testId) {
    conditions.push(eq(integrityTestResults.testId, testId));
  }
  ...
  // Calcular tendências e análise agregada
  ...
  const analysis = results.map((r) => ({
    ...
    behavioralAnalysis: r.result.behavioralAnalysis,
