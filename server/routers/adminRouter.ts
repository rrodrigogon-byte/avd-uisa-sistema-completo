...
export const adminRouter = router({
   ...
   */
  getEmailStats: protectedProcedure.input(z.object({})).query(async ({ ctx }) => {
   ...
   */
  getSmtpConfig: protectedProcedure.input(z.object({})).query(async ({ ctx }) => {
    if (ctx.user.role !== 'admin') {
      throw new TRPCError({
        code: 'FORBIDDEN',
   ...
   */
  getEmailHistory: protectedProcedure.input(z.object({})).query(async ({ ctx }) => {
    if (ctx.user.role !== 'admin') {
      throw new TRPCError({
        code: 'FORBIDDEN',
