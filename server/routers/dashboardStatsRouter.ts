...
export const dashboardStatsRouter = router({
   ...
   */
  getByDepartment: protectedProcedure.query(async () => {
    const db = await getDb();
    if (!db) return [];
   ...
   */
  getByPosition: protectedProcedure.query(async () => {
    const db = await getDb();
    if (!db) return [];
   ...
   */
  getByTenure: protectedProcedure.query(async () => {
    const db = await getDb();
    if (!db) return [];
   ...
   */
  getAverageTenure: protectedProcedure.query(async () => {
    const db = await getDb();
