...
export const employeesRouter = router({
   ...
   */
  sendCredentials: protectedProcedure
    .input(z.object({ userId: z.number() }))
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== "admin") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Apenas administradores podem enviar credenciais",
        });
      }
      ...
      try {
        await sendEmail({
          ...
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <p>Suas credenciais de acesso foram configuradas.</p>
              <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
                <p style="margin: 0;"><strong>Email:</strong> ${user[0].email}</p>
                <p style="margin: 10px 0 0 0;"><strong>Perfil:</strong> ${user[0].role}</p>
              </div>
              <p>Para acessar o sistema, clique no botão abaixo:</p>
              <div style="text-align: center; margin: 30px 0;">
                <a href="${ENV.viteOauthPortalUrl}" 
        ...
        return { success: true, message: "Credenciais enviadas com sucesso" };
      } catch (error) {
        console.error("[sendCredentials] Erro:", error);
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
