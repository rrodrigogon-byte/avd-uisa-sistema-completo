...
export const avdUisaRouter = router({
   ...
   */
  myPendingEvaluations: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database not available" });
  ...
  /**
   * Iniciar avaliações em lote para múltiplos subordinados
   */
  startBatchEvaluations: protectedProcedure
    .input(z.object({
      cycleId: z.number(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Verificar se é líder/gestor
      if (ctx.user.role !== "admin" && ctx.user.role !== "rh" && ctx.user.role !== "gestor") {
        throw new TRPCError({ code: "FORBIDDEN", message: "Apenas líderes podem iniciar avaliações em lote" });
      }

      // Verificar se o ciclo existe e está ativo
      ...
      if (!cycle) {
        throw new TRPCError({ code: "NOT_FOUND", message: "Ciclo de avaliação não encontrado" });
      }

      if (cycle.status !== "ativo") {
        throw new TRPCError({ code: "BAD_REQUEST", message: "O ciclo de avaliação não está ativo" });
      }

      const results = {
        created: 0,
        skipped: 0,
        errors: [] as { employeeId: number; error: string }[],
      };
      ...
      for (const employeeId of input.employeeIds) {
        try {
          ...
          if (!employee) {
            results.errors.push({ employeeId, error: "Funcionário não encontrado" });
            continue;
          }
          ...
          if (employee.userId) {
            await db.insert(notifications).values({
              ...
              type: "avaliacao_iniciada",
              title: "Nova Avaliação Disponível",
              message: `Uma nova avaliação do ciclo "${cycle.name}" foi iniciada para você. Complete sua autoavaliação.`,
              link: "/avaliacoes",
              read: false,
            });
          }
          ...
          results.errors.push({ employeeId, error: String(error) });
        }
      }
      ...
      return {
        success: true,
        ...results,
        total: input.employeeIds.length,
      };
    }),

  /**
   * Listar subordinados disponíveis para avaliação em lote
   */
  getSubordinatesForBatchEvaluation: protectedProcedure
    .input(z.object({
      cycleId: z.number(),
    }))
    .query(async ({ ctx, input }) => {
      ...
      const { employeeHierarchy } = await import("../drizzle/schema");

      // Buscar o funcionário vinculado ao usuário logado
      const [currentEmployee] = await db
        .select()
      ...
      if (!currentEmployee) {
        return { subordinates: [], hasExistingEvaluations: [] };
      }

      // Buscar subordinados (coordenador, gestor ou diretor)
      const subordinates = await db
        .select()
        .from(employeeHierarchy)
        .where(
          or(
            eq(employeeHierarchy.coordinatorChapa, currentEmployee.employeeChapa),
            eq(employeeHierarchy.managerChapa, currentEmployee.employeeChapa),
            eq(employeeHierarchy.directorChapa, currentEmployee.employeeChapa)
          )
        )
        .orderBy(employeeHierarchy.employeeName);

      // Buscar IDs dos funcionários na tabela employees
      const subordinatesWithIds = await Promise.all(
        subordinates.map(async (sub) => {
          const [emp] = await db
            ...
            .where(
              or(
                eq(employees.chapa, sub.employeeChapa),
                eq(employees.email, sub.employeeEmail || "")
              )
            )
            .limit(1);
          ...
          return {
            ...sub,
            employeeTableId: emp?.id || null,
          };
        })
      );

      // Buscar avaliações existentes para este ciclo
      const existingEvaluations = await db
        .select({ employeeId: performanceEvaluations.employeeId })
        .from(performanceEvaluations)
        .where(eq(performanceEvaluations.cycleId, input.cycleId));

      const hasExistingEvaluations = existingEvaluations.map(e => e.employeeId);

      return {
        subordinates: subordinatesWithIds.filter(s => s.employeeTableId !== null),
        hasExistingEvaluations,
      };
    }),

  /**
   * Obter informações do funcionário vinculado ao usuário logado
   */
  getLinkedEmployee: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database not available" });

    const { employeeHierarchy } = await import("../drizzle/schema");

    // Buscar na tabela employeeHierarchy pelo email
    const [hierarchyEmployee] = await db
      .select()
      .from(employeeHierarchy)
      .where(eq(employeeHierarchy.employeeEmail, ctx.user.email || ""))
      .limit(1);

    // Buscar na tabela employees pelo email ou userId
    const [employee] = await db
      .select()
      .from(employees)
      .where(
    ...
    if (hierarchyEmployee) {
      // Verificar se é líder em algum nível
      const [hasSubordinates] = await db
        .select({ count: sql<number>`count(*)` })
        .from(employeeHierarchy)
        .where(
          or(
            eq(employeeHierarchy.coordinatorChapa, hierarchyEmployee.employeeChapa),
            eq(employeeHierarchy.managerChapa, hierarchyEmployee.employeeChapa),
            eq(employeeHierarchy.directorChapa, hierarchyEmployee.employeeChapa),
            eq(employeeHierarchy.presidentChapa, hierarchyEmployee.employeeChapa)
          )
        );

      isLeader = (hasSubordinates?.count || 0) > 0;

      // Determinar nível
      if (hierarchyEmployee.presidentChapa === hierarchyEmployee.employeeChapa) {
        hierarchyLevel = "presidente";
      } else if (hierarchyEmployee.directorChapa === hierarchyEmployee.employeeChapa) {
        hierarchyLevel = "diretor";
      } else if (hierarchyEmployee.managerChapa === hierarchyEmployee.employeeChapa) {
        hierarchyLevel = "gestor";
      } else if (hierarchyEmployee.coordinatorChapa === hierarchyEmployee.employeeChapa) {
    ...
    return {
      ...
      hierarchyEmployee: hierarchyEmployee || null,
      hierarchyLevel,
      isLeader,
      isLinked: !!hierarchyEmployee || !!employee,
    };
  }),
});
